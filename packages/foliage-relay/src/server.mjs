...
const publicBase =
  getArgumentValue('--public-base') ?? process.env.FOLIAGE_RELAY_PUBLIC_BASE ?? `http://127.0.0.1:${port}`
const sessionTtlMs = Number.parseInt(process.env.FOLIAGE_RELAY_SESSION_TTL_MS ?? '86400000', 10)
...
const server = createServer((request, response) => {
  ...
  if (url.pathname === '/api/health' && request.method === 'GET') {
    sendJson(response, 200, { ok: true, service: 'foliage-relay' }, request)
    return
  }
...
server.listen(port, () => {
  console.log(`foliage-relay listening on ${port}`)
