...
const getLeafmarkRoot = () => {
  const appRoot = process.env.FOLIAGE_APP_ROOT ?? process.cwd()
...
const resolveOutputRoot = (workspaceRoot, outputDir = '') => {
...
const buildCliOptions = (options = {}) => ({
  ...
  wantHelp: false,
  outputFormat: options.outputFormat === 'docx' ? 'docx' : 'pdf',
...
const discoverProjectWorkspace = async (workspaceRoot, absoluteInputRoot, outputDir = '') => {
  const { discoverWorkspace } = await importLeafmark('workspace/workspace.js')
  return discoverWorkspace(absoluteInputRoot, resolveOutputRoot(workspaceRoot, outputDir))
}
...
const getProjectContext = async (workspaceRoot, relativeFolder = '') => {
  ...
  const workspace = await discoverProjectWorkspace(workspaceRoot, absolutePath)
...
const collectOutputs = async (workspaceRoot, workspace, outputFormat = 'pdf') => {
  const distDir = workspace.outputRoot
  const primaryFilename = outputFormat === 'docx' ? 'output.docx' : 'output.pdf'
  const primaryAbs = path.join(distDir, primaryFilename)
  ...
  try {
    ...
    if (outputFormat === 'docx') {
      outputs.docx = toRelativePath(workspaceRoot, primaryAbs)
    } else {
      outputs.pdf = toRelativePath(workspaceRoot, primaryAbs)
    }
  } catch {
    // Primary output was not produced.
  }
...
export const listLeafmarkFolders = async (workspaceRoot) => {
  const { isLeafmarkProject } = await importLeafmark('workspace/workspace.js')
  const ignored = new Set(['.git', '.next', '.turbo', '.foliage', 'node_modules', 'dist'])
  const folders = []
  const seen = new Set()
...
export const buildLeafmarkProject = async (workspaceRoot, relativeFolder = '', options = {}) => {
  ...
  const workspace = await discoverProjectWorkspace(
    workspaceRoot,
